...
class TuiGatewayBridge {
  ...
  async chatStart({ message, profile, session_id, model }) {
    if (!this.ready) {
    ...
    if (!session_id) {
      ...
      try {
        await this.request('session.resume', { session_id });
      } catch {
        // Session might not exist, create new
        const result = await this.request('session.create', { model: model || undefined });
        session_id = result.session_id;
      }
    }
    ...
    // Send prompt
    await this.request('session.sendPrompt', { session_id, text: message });
